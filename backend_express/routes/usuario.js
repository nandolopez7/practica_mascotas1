 ...
 */
 router.get('/toy', function (req, res, next) {
  connect(function (err, client, done) {
    ...
    //use the client for executing the query
    client.query('SELECT * FROM Toy;', function (err, result) {
      //call `done(err)` to release the client back to the pool (or destroy it if there is an error)
      done(err);
...
router.post('/pet', function (req, res, next) {
  connect(function (err, client, done) {
    if (err) {
      ...
      //use the client for executing the query
    client.query(`INSERT INTO  Pet(pet_name, age,type_ID) VALUES ('${req.body.nombre_mascota}', '${req.body.edad}','${req.body.tipo}');`, function (err, result) {
    //call `done(err)` to release the client back to the pool (or destroy it if there is an error)
      done(err);
      if (err) {
        return console.error('error running query', err);
 ...
 * !Antes de crearlo debería verificar si ya existe.
 */
 router.post('/toy', function (req, res, next) {
  connect(function (err, client, done) {
    if (err) {
      return console.error('error fetching client from pool', err);
    }
    //use the client for executing the query
    client.query(`INSERT INTO  Toy(toy_name, color,pet_ID) VALUES ('${req.body.nombre_juguete}', '${req.body.color}','${req.body.pet}');`, function (err, result) {
    //call `done(err)` to release the client back to the pool (or destroy it if there is an error)
    done(err);
    if (err) {
      return console.error('error running query', err);
